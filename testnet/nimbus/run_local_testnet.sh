...
DEPOSITS_FILE="${DATA_DIR}/validators/deposit_data.json"
...
if [[ $USE_GANACHE == "0" ]]; then
  ...
  ~/nimbus-eth2/build/nimbus_beacon_node createTestnet \
    --data-dir="${DATA_DIR}" \
...
dump_logs() {
  LOG_LINES=20
  for LOG in "${DATA_DIR}"/logs/log*.txt; do
...
dump_logtrace() {
  if [[ "$ENABLE_LOGTRACE" == "1" ]]; then
    find "${DATA_DIR}" -maxdepth 1 -type f -regex '.*/logs/log[0-9]+.txt' | sed -e"s/${DATA_DIR}\//--nodes=/" | sort | xargs ./build/logtrace asr --log-dir="${DATA_DIR}" || true
  fi
}
...
for NUM_NODE in $(seq 0 $(( NUM_NODES - 1 ))); do
  ...
  ~/nimbus-eth2/build/nimbus_beacon_node \
    ...
    ${EXTRA_ARGS} \
    > "${DATA_DIR}/logs/log${NUM_NODE}.txt" 2>&1 &
  ...
  if [ "${USE_VC:-}" == "1" ]; then
    ./build/nimbus_validator_client \
      ...
      > "${DATA_DIR}/logs/log_val${NUM_NODE}.txt" 2>&1 &
  fi
done
