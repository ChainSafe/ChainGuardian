...
export class LighthouseValidatorApiClient implements IValidatorApi {
    ...
    public async produceAttestation(
        ...
        const response = await this.client.get<Json>(LighthouseRoutes.GET_ATTESTATION, {
            params: {
                slot,
    ...
    public async produceBlock(slot: Slot, proposerPubKey: BLSPubkey, randaoReveal: Uint8Array): Promise<BeaconBlock> {
        const response = await this.client.get<Json>(LighthouseRoutes.GET_BLOCK, {
            params: {
                slot,
    ...
    public async publishAttestation(attestation: Attestation): Promise<void> {
        await this.client.post(LighthouseRoutes.PUBLISH_ATTESTATION, [
            [
    ...
    public async subscribeCommitteeSubnet(
        ...
        await this.client.post(LighthouseRoutes.SUBSCRIBE_TO_COMMITTEE_SUBNET, [
            {
    ...
    public async produceAggregateAndProof(
        ...
        const response = await this.client.get<Json>(LighthouseRoutes.GET_AGGREGATED_ATTESTATION, {
            params: {
